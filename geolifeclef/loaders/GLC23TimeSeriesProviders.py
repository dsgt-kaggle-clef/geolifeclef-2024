...
class TimeSeriesProvider(object):
    ...
    def plot_ts(self, item):
        ...
        if self.nb_layers == 1:
            ...
            plt.plot(
                range(eos_start_ind),
                ts_[0, 0, :eos_start_ind],
                "-.",
                c="blue",
                marker="+",
            )
            plt.plot(
                range(eos_start_ind, ts_.shape[2]),
                ts_[0, 0, eos_start_ind:],
                "",
                c="red",
                marker="+",
            ...
            plt.xticks(
                list(range(ts_.shape[2]))[::4] + [ts_.shape[2] - 1],
            ...
            for i in range(self.nb_layers):
                ...
                eos_start_ind = (
                    eos_start_ind[0] if eos_start_ind != [] else ts_.shape[0] - 1
                )
                ...
                axs[i].plot(
                    range(eos_start_ind, ts_.shape[0]),
                    ts_[eos_start_ind:],
                    "",
                    c="red",
                    marker="+",
                ...
                axs[i].set_xticks(
                    list(range(ts_.shape[0]))[::4] + [ts_.shape[0] - 1],
...
class CSVTimeSeriesProvider(TimeSeriesProvider):
    def __init__(
        ...
        self.ts_data = pd.read_csv(ts_data_path, sep=";").set_index(
            self.ts_id, drop=False
        )
        self.ts_data = self.ts_data.replace("eos", eos_replace_value).astype(np.int16)
        self.eos_replace_value = eos_replace_value
        self.max_sequence, self.min_sequence = self.get_min_max_sequence(
            eos_replace_value
        )
        self.nb_layers = 1
        if not features_col:
            self.features_col = list(self.ts_data.columns[1:])
        elif len(set(features_col).intersection(self.ts_data.columns)) == len(
            features_col
        ):
            ...
            raise KeyError(
                "Some values in `features_col` do not match the `ts_data` column names."
            )
    ...
    def get_min_max_sequence(self, eos_replace_value):
        ...
        max_row = self.ts_data.loc[
            (self.ts_data == eos_replace_value).sum(axis=1).idxmax()
        ]
    ...
    def __init__(
        ...
        if select:
            ...
            if len(ts_paths) != len(select):
                logging.warn(
                    f"Could not find all files based on `select`. Loading only the ones which names match... (see the `ts_paths` attribute for the complete list)"
                )
        self.ts_paths = ts_paths
        self.ts_providers = [
            CSVTimeSeriesProvider(
                root_path + path,
                normalize=normalize,
                ts_id=ts_id,
        ...
        self.bands_names = list(
            itertools.chain.from_iterable(
                [provider.bands_names for provider in self.ts_providers]
            )
        )
        self.min_sequence = min(
            list(
                itertools.chain.from_iterable(
                    [[ts_.min_sequence] for ts_ in self.ts_providers]
                )
            )
        )
        self.max_sequence = max(
            list(
                itertools.chain.from_iterable(
                    [[ts_.max_sequence] for ts_ in self.ts_providers]
                )
            )
        )
