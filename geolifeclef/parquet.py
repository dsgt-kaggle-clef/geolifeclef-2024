...
from utils import get_local_spark

class LoadDataset(ExternalProgramTask):
  script_path = "script/download_dataset.sh"
  url = luigi.Parameter(default="gs://dsgt-clef-geolifeclef-2024/data/raw/EnvironmentalRasters/Climate/BioClimatic_Average_1981-2010")
...
class PreProcess(ExternalProgramTask):
  ...
  def program_args(self):
    return [self.script_path, self.process_path, self.download_path, str(self.tile_size), str(self.tile_size)]
